...
update_free_chips(){
        ...
        this.users.creditbyUser({userid:this.state.userdetails.id,fillAmount:this.state.Chips},'creditAmountBySuperAdmin',pdata=>{
          ...
          this.users.debitbyUser({userid:this.state.userdetails.id,fillAmount:this.state.Chips},'debitAmountBySuperAdmin',pdata=>{
...
view_change_passs(userdetails){
  ...
  submit_info(){
    ...
    this.users.updateUserSportsInfo({id:this.state.userdetails.id,cricketmaxStacks:this.state.max_stake,cricketminStacks:this.state.min_stake,cricketmaxProfit:this.state.max_profit,cricketmaxLoss:this.state.max_loss,cricketPreInplayProfit:this.state.PIP,cricketPreInplayStack:this.state.PIS,cricketmaxOdds:this.state.max_odds,cricketminOdds:this.state.min_odds},data=>{
  ...
  submit_userInfo(){
    ...
    this.users.updateMyprofile({userName:this.state.userdetails.userName,Name:this.state.Name},data=>{
...
closechipWithdrawalpopup(){
 ...
 save(){
    let data; let message;let path;
if(this.state.masterUName){
  data={userName:this.state.userName,Name:this.state.name,password:this.state.password,admin:this.state.masterUName,Master:true,Commission:this.state.partner}
...
setAction(){
  if(this.state.useraction == 'mstrlock-0'){
    ...
    alert('Supermaster locked successfully');
    window.location.reload();
  });
}
else if(this.state.useraction == 'mstrlock-1'){
  this.users.lockunlock({userName:this.state.mName},data=>{
  alert('Supermaster unlocked successfully');
  window.location.reload();
   });
}
...
updatePass(){
  if(this.state.username && this.state.confirm_password ){
    this.users.changePassword({userName:this.state.username,password:this.state.confirm_password},data=>{
...
handleSearch = (e) => {
  ...
  const updateList = dataArray.filter(user => user.userName.toLowerCase().includes(searchUser));
...
if(this.state.data && this.state.data.length>0){
  ...
  <tr id="user_row_152052">
    ...
    <td className=" " style={{ paddingBottom: '0px' }}>
      <span className="m-bg">
        <a href={'/master/'+item.userName} title="View Child"> {item.userName}</a>&nbsp;
        {item.status ? (<i class="fa fa-user fa_custom fa-2x" title="locked" aria-hidden="true" style={{color:'red'}}></i>) : ''}
        {item.enableBetting ? (<i class="fa fa-lock fa_custom fa-2x" title="Betting Locked" aria-hidden="true" style={{color:'red'}}></i>) : '' }
      </span>
    </td>
    <td>BETFUN360</td>
    ...
    <td className="last">
      <span className="dropdown">
        ...
        <ul className="dropdown-menu">
          <li>
            <a className data-toggle="modal" masterName={item.userName} data-target="#commonpopup" onClick={() => this.addUser(item.userName)}><span>Add User</span></a>
          </li>
          <li>   <Link to={'/cacstatement/'+item.userName}>Statement</Link>
            </li>
          <li>
          <Link to={'/profitloss/'+item.userName}>Profit Loss</Link>
            </li>
          <li>
            <a className href="javascript:;" title="View Account Info" onClick={()=>this.view_account(item)}><span>View Info</span></a>
          </li>
          <li> <a className href="javascript:;" title="Change Password" onClick={() => this.view_change_passs(item)}><span>Change Password</span></a></li>
          <li> <a className href="javascript:;" title="Free Chip In Out"  onClick={() => this.openChipDeposit(item)}><span>Free Chip Deposit</span></a></li>
          <li>
            <a className href="javascript:;" title="Free Chip In Out" onClick={() => this.openChipWithdrawal(item)}><span>Free Chip Withdrawal</span></a>
          </li>
        </ul>
      </span>
    </td>
          ...
          <div className="right_col" role="main">
            <div className="col-md-12">
              <div className="title_new_at">
                ...
                <select className="user-select" style={{ color: 'black', margin:"auto" }} onchange="perPage(this.value)">
                ...
                <select className="user-mobile custom-user-select" name ="useraction" id="useraction" onChange={this.handleChange} 
                  ...
                  <div className="modal-dialog" role="document">
                    <div className="modal-content">
                      <div className="modal-header">
                        <h5 className="modal-title" id="exampleModalLabel">Add Master</h5>
                      ...
                      <div class="modal-body">
                        <div class="row">
                            <div class="col-md-4 col-xs-6">
                              ...
                              <span id="left_master_nameN" class="errmsg"></span>
                            </div>
                            <div class="col-md-4 col-xs-6">
                              <label> Registration Data </label>
                              <input type="text" name="FromDate" class="form-control" id="Fleft_romDate" autocomplete="off" value="2020-07-14" readonly=""/>
                            ...
                            <div class="col-md-4 col-xs-6">
                              ...
                              <input type="number" name="partner" onChange={this.handleChange} class="form-control" id="left_partner" placeholder="0" max="100" min="0" autocomplete="off" onkeypress="return isNumberKey(event)"/>
                            ...
                            <div class="col-md-4 col-xs-6">
                              ...
                              <input type="number" name="partnershipCasino" class="form-control" id="left_partnershipCasino" max="100" min="0" autocomplete="off" value="0" onkeypress="return isNumberKey(event)"/>
                            ...
                            <div class="col-md-4 col-xs-6">
                              ...
                              <input type="number" name="partnershipLiveTennPatti" class="form-control" id="left_partnershipLiveTennPatti" max="100" min="0" autocomplete="off" value="0" onkeypress="return isNumberKey(event)"/>
                              ...
                              <button data-dismiss="modal" type="button" class="red_button" onclick="reset_f();">Cancel</button>
                            </div>
                        </div>
                      </div>
                  ...
                  <table className="table table-striped jambo_table bulk_action" id="datatabless">
                    <thead>
                      <tr className="headings">
                        ...
                        <th>Credit&nbsp;Limit</th>
                        <th>Credit&nbsp;given</th>
                        <th>Balance <a href="#" className="glyphicon glyphicon-arrow-up"> </a>
                          <a href="#" className="glyphicon glyphicon-arrow-down" />
                        </th>
                    ...
                    <tfoot>
                      <tr><td colSpan={5} /><td colSpan={9}>Total Balance: {this.state.totalBalance}</td></tr>
                    </tfoot>
                  </table>
               ...
               <div className="row">
                  ...
                  <div className="col-md-4 col-xs-6">
                    ...
                    <div className="col-md-12 col-xs-6 modal-footer">
                     ...
                     <button data-dismiss="modal" type="button" className="red_button" onclick="reset_f();">Cancel</button>
                  </div>
               </div>
            </div>
                    </div>
...
<div class="modal-header">
         ...
         <div class="modal-body">
         <div class="row">
         <div class="sub_heading"><span id="tital_change">User  </span> </div>
           <form>
                    <div class="col-md-4 col-xs-6">
                        <label>User ID</label>
                     ...
                     <div class="col-md-12 col-xs-12 modal-footer"> 
                        <button type="button" class="blue_button submit_user_setting" onClick={()=>this.submit_userInfo()}>Update</button>  
                     </div>
            </form>
          </div>
